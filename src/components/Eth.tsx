...
import { createEthAccount, addEthAccount } from '../utils/walletEth';
import type { EthWalletAccount } from '../utils/walletEth';
import { Navbar } from './Navbar';
import axios from 'axios';

export const Eth = () => {
    const [mnemonic, setMnemonic] = useState('');
    const [accounts, setAccounts] = useState<EthWalletAccount[]>([]);
    const [showMnemonic, setShowMnemonic] = useState(false);
    const [balance, setBalance] = useState<string | null>(null);

    const handleCreateWallet = () => {
        const { mnemonic, accounts } = createEthAccount();
    ...
    const handleAddAccount = () => {
        ...
        const newAccount = addEthAccount(mnemonic, accounts);
        setAccounts([...accounts, newAccount]);
    };

    const getEthBalance = async (publicKey: string): Promise<string> => {
        const body = {
            "jsonrpc": "2.0",
            "id": 1,
        ...
        try {
            const res = await axios.post('https://eth-mainnet.g.alchemy.com/v2/Ub9bKtLmFHitTHFeiZEGD', body)
            //@ts-ignore
        ...
        <>
            ...
            <div className="bg-slate-900 py-10 px-8 min-h-screen">
                <div className="max-w-5xl mx-auto mt-4">
                    ...
                    <h1 className="font-barlow text-4xl font-bold text-white px-3 leading-tight transition-all duration-150">
                        Ethereum Wallet
                    </h1>
                            ...
                            <div className="mt-10 space-y-4">
                                {accounts.map((acc) => (
                                    ...
                                    >
                                        ...
                                        <p
                                            onClick={async () => {
                                                const b = await getEthBalance(acc.publicKey);
                                                setBalance(b);
                                            }}
                                            className="cursor-pointer text-indigo-400 hover:underline"
                                        >
                                            {balance ? `Balance: ${balance} ETH` : 'Show Balance'}
                                        </p>
                                    </div>
                                ))}
                            </div>
