...
export const Solana = () => {
    ...
    const getSolBalance = async (publicKey: string): Promise<string> => {
        
        try {
            const connection = new Connection('https://solana-mainnet.g.alchemy.com/v2/Ub9bKtLmFHitTHFeiZEGD');
                            ...
                            <div className="mt-10 space-y-4">
                                {accounts.map((acc) => (
                                    ...
                                    >
                                        ...
                                        <p
                                            className="cursor-pointer text-indigo-400 mt-2 hover:underline"
                                            onClick={()=>{getSolBalance(acc.publicKey)}}
                                        >
                                            {balance ? `Balance: ${balance} SOL` : 'Show Balance'}
                                        </p>
                                    </div>
                                ))}
                            </div>
